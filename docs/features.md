...
| C/C++ | `features.cpp` | Instala GCC/G++, herramientas de build, GDB y clang-tools. |
...
`containers.windowsVm`. Browser, C/C++, Git, Go, Holodeck, Lean, Node.js,
Python, Rust y VSCodium permanecen apagadas en la capa NixOS porque el perfil
`developer` de Home Manager ya instala sus equivalentes. `graphics` también
queda apagada hasta elegir explícitamente el driver de la máquina.
