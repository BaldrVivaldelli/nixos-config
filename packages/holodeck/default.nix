...
writeShellApplication {
  ...
  text = ''
    ...
    # These variables only affect provider CLIs started by Holodeck. The
    # detached launcher keeps browser diagnostics out of the auth flow without
    # changing the user's browser outside this command.
    export GH_BROWSER=${quietBrowser}/bin/holodeck-open-browser
    export BROWSER=${quietBrowser}/bin/holodeck-open-browser
